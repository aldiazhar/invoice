...
### 1. **Dual Morph Relationship System**
Dua hubungan polymorphic yang powerful:
- **Payer Morph**: User, Agent, Company (siapa yang membayar)
- **Invoiceable Morph**: TopUp, Registration, Service (apa yang dibayar)

### 2. **Fluent Builder Pattern**
Syntax yang mirip natural language dengan flexible item format:
...
### 3. **Multi-Directional Invoice Creation**
Buat invoice dari siapa pun dan untuk apa pun:

```php
// Dari sisi Payer
$user->invoice()->pay($topup)

// Dari sisi Invoiceable  
$topup->bill()->to($user)
...
### 4. **Smart Callback System**
Automated actions setelah pembayaran:
...
$invoice = $agent->invoice()
    ->pay($commissionFee)
    ->item('Platform Commission', 25.00)
    ->discount(5.00)
    ->create();
```
